...
// N-tier supply mapping with cascading risk propagation
// Feature: n-tier-supply-mapping
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: N-tier supply mapping with cascading risk propagation.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'n-tier-supply-mapping', payload, result);
    res.json({ success: true, feature: 'n-tier-supply-mapping', ...result });
  } catch (err) {
    console.error('n-tier-supply-mapping analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: N-tier supply mapping with cascading risk propagation. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'n-tier-supply-mapping', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'n-tier-supply-mapping', title: 'N-tier supply mapping with cascading risk propagation', project: 'AIVendorRiskPerformanceScorer' });
});
