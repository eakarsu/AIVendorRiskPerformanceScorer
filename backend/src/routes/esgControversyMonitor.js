...
// ESG controversy monitoring via real-time news scanning
// Feature: esg-controversy-monitor
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: ESG controversy monitoring via real-time news scanning.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'esg-controversy-monitor', payload, result);
    res.json({ success: true, feature: 'esg-controversy-monitor', ...result });
  } catch (err) {
    console.error('esg-controversy-monitor analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: ESG controversy monitoring via real-time news scanning. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'esg-controversy-monitor', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'esg-controversy-monitor', title: 'ESG controversy monitoring via real-time news scanning', project: 'AIVendorRiskPerformanceScorer' });
});
