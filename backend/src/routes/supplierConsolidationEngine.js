...
// Supplier consolidation recommendation engine
// Feature: supplier-consolidation-engine
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Supplier consolidation recommendation engine.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'supplier-consolidation-engine', payload, result);
    res.json({ success: true, feature: 'supplier-consolidation-engine', ...result });
  } catch (err) {
    console.error('supplier-consolidation-engine analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Supplier consolidation recommendation engine. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'supplier-consolidation-engine', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'supplier-consolidation-engine']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'supplier-consolidation-engine', title: 'Supplier consolidation recommendation engine', project: 'AIVendorRiskPerformanceScorer' });
});
