...
// Risk-adjusted pricing flagging high-risk suppliers for negotiation leverage
// Feature: risk-adjusted-pricing
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIVendorRiskPerformanceScorer" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Risk-adjusted pricing flagging high-risk suppliers for negotiation leverage.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'risk-adjusted-pricing', payload, result);
    res.json({ success: true, feature: 'risk-adjusted-pricing', ...result });
  } catch (err) {
    console.error('risk-adjusted-pricing analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIVendorRiskPerformanceScorer". Return strict JSON only.',
      `Feature: Risk-adjusted pricing flagging high-risk suppliers for negotiation leverage. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'risk-adjusted-pricing', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'risk-adjusted-pricing', title: 'Risk-adjusted pricing flagging high-risk suppliers for negotiation leverage', project: 'AIVendorRiskPerformanceScorer' });
});
