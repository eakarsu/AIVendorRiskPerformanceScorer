...
// Supply chain network risk identifying single points of failure
// Feature: supply-chain-network-risk
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Supply chain network risk identifying single points of failure.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'supply-chain-network-risk', payload, result);
    res.json({ success: true, feature: 'supply-chain-network-risk', ...result });
  } catch (err) {
    console.error('supply-chain-network-risk analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Supply chain network risk identifying single points of failure. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'supply-chain-network-risk', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'supply-chain-network-risk', title: 'Supply chain network risk identifying single points of failure', project: 'AIVendorRiskPerformanceScorer' });
});
